...
const iconDefinitions = [
    ...
    },
    {
        id: "docs",
        label: "Google Docs",
        matches: (url) => url.hostname === "docs.google.com" &&
    ...
    },
    {
        id: "forms",
        label: "Google Forms",
        matches: (url) => url.hostname === "docs.google.com" && url.pathname.startsWith("/forms"),
...
function removePageFavicons() {
    const faviconLinks = document.head.querySelectorAll("link[rel*='icon' i]");
...
function getOrCreateManagedLink() {
    const existing = document.head.querySelector(`link#${MANAGED_LINK_ID}[${MANAGED_ATTR}]`);
    if (existing) {
        return existing;
    }
...
function mutationTouchesFavicon(mutation) {
